...
def main():
    # 解析多卡训练参数（将local_rank改为local-rank）
    parser = argparse.ArgumentParser()
    parser.add_argument('--local-rank', type=int, default=0)  # 注意这里是短横线
    args = parser.parse_args()

    # 后续代码中使用 args.local_rank 访问（变量名仍为下划线，不影响）
    ...
    target_size = 32
    template_size = 128
    batch_size = 1024  # 单卡batch_size，总batch_size = 单卡batch_size * 卡数
    epochs = 500
    lr = 2e-4 * torch.cuda.device_count()  # 按卡数线性缩放学习率
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    
    # 初始化分布式环境
    dist.init_process_group(
        backend='nccl',  # cuda使用hccl后端，GPU使用nccl
        init_method='env://'
    )
    local_rank = args.local_rank
    torch.cuda.set_device(local_rank)  # 设置当前进程使用的设备
    ...
    sampler = DistributedSampler(dataset, shuffle=True)  # 不同卡加载不同数据分片
    dataloader = DataLoader(
        dataset,
        batch_size=batch_size,
        sampler=sampler,  # 替换原来的shuffle=True
        num_workers=4,
        pin_memory=True
    )
    
    # 模型初始化与DDP包装
    model = MultiScaleUNet(target_size).to(local_rank)
    model = DDP(model, device_ids=[local_rank], output_device=local_rank)  # 多卡包装
    ...
    for epoch in range(epochs):
        ...
        for step, (cond, target) in enumerate(dataloader):
            try:
                cond = cond.to(local_rank, dtype=torch.float32)
                target = target.to(local_rank, dtype=torch.float32)
                
                # 随机时间步
                t = torch.randint(0, diffusion.timesteps, (target.size(0),), device=local_rank)
        ...
        dist.all_reduce(total_loss_tensor, op=dist.ReduceOp.SUM)  # 所有卡的损失求和
        avg_loss = total_loss_tensor.item() / (len(dataloader) * dist.get_world_size())  # 平均到单卡
        ...
        if is_main_process and epoch % 2 == 0:
            try:
                ...
                with torch.no_grad():
                    ...
                    cond_batch = cond_batch[:4].to(local_rank)
                    target_batch = target_batch[:4].to(local_rank)
