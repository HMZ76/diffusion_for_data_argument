...
def load_image_tensor(path, size=None, device='cuda'):
...
def load_image_for_fid(path, size=(299, 299), device='cuda'):
...
# ===================== 核心评估函数 =====================
def calculate_lpips(gen_paths, target_paths, device='cuda'):
    """
    计算LPIPS（感知相似度）
...
def calculate_fid(gen_folder, target_folder, device='cuda', batch_size=32):
    """
...
def main():
    ...
    parser.add_argument("--device", type=str, default='cuda', help="设备（cuda/cpu，建议使用GPU）")
    ...
    if args.device == 'cuda' and not torch.cuda.is_available():
        print("警告：cuda不可用，自动切换到CPU（计算速度会很慢！）")
        args.device = 'cpu'
